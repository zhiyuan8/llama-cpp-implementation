...
// Macro definitions for handling deprecated functions across different compilers
#ifdef __GNUC__
...
// DLL export/import macros for Windows and visibility settings for other platforms
...
#define WHISPER_SAMPLE_RATE 16000    // Audio sample rate (16 kHz)
#define WHISPER_N_FFT       400      // FFT size for audio processing
#define WHISPER_N_FFT_HALF  (WHISPER_N_FFT / 2 + 1)  // Half of FFT size plus one
#define WHISPER_HOP_LENGTH  160      // Number of samples between successive FFT windows
#define WHISPER_CHUNK_SIZE  30       // Size of audio chunks in seconds
#define WHISPER_N_SAMPLES   (WHISPER_SAMPLE_RATE * WHISPER_CHUNK_SIZE)  // Total samples per chunk
...
extern "C" {
    ...
    struct whisper_context;  // Main context holding model and processing data
    struct whisper_state;    // State information for processing
    struct whisper_full_params;  // Parameters for full transcription

    // Basic type definitions
    typedef int32_t whisper_pos;     // Position type
    typedef int32_t whisper_token;   // Token identifier type
    typedef int32_t whisper_seq_id;  // Sequence identifier type

    // Enum for alignment heads preset selection
    enum whisper_alignment_heads_preset {
        WHISPER_AHEADS_NONE,           // No alignment heads
        WHISPER_AHEADS_N_TOP_MOST,     // Use N top-most text layers
        WHISPER_AHEADS_CUSTOM,         // Custom alignment heads configuration
        WHISPER_AHEADS_TINY_EN,        // Preset for tiny English model
        WHISPER_AHEADS_TINY,           // Preset for tiny multilingual model
        WHISPER_AHEADS_BASE_EN,        // Preset for base English model
        WHISPER_AHEADS_BASE,           // Preset for base multilingual model
        WHISPER_AHEADS_SMALL_EN,       // Preset for small English model
        WHISPER_AHEADS_SMALL,          // Preset for small multilingual model
        WHISPER_AHEADS_MEDIUM_EN,      // Preset for medium English model
        WHISPER_AHEADS_MEDIUM,         // Preset for medium multilingual model
        WHISPER_AHEADS_LARGE_V1,       // Preset for large v1 model
        WHISPER_AHEADS_LARGE_V2,       // Preset for large v2 model
        WHISPER_AHEADS_LARGE_V3,       // Preset for large v3 model
    };
    ...
    // Structure for multiple alignment heads
    ...
    struct whisper_context_params {
        bool  use_gpu;
        bool  flash_attn;
        int   gpu_device;  // CUDA device, can specify which GPU to use
    ...
    typedef struct whisper_model_loader {
        // This is a generic pointer that holds the context/state needed by the loader
        // It could point to a file handle, memory buffer, or any other data source
        void * context;    

        // These are function pointers - they declare the signature of functions that will be used
        // to perform loading operations
        
        // Read function pointer:
        // - Takes a context (ctx), a buffer to write to (output), and number of bytes to read (read_size)
        // - Returns the number of bytes actually read
        size_t (*read)(void * ctx, void * output, size_t read_size);
        
        // Seek function pointer:
        // - Takes a context (ctx) and position to seek to (offset)
        // - Moves the reading position to specified offset
        void   (*seek)(void * ctx, size_t offset);
        
        // End-of-file function pointer:
        // - Takes a context (ctx)
        // - Returns true if we've reached the end of the data
        bool   (*eof)(void * ctx);
        
        // Close function pointer:
        // - Takes a context (ctx)
        // - Cleans up resources when done loading
        void   (*close)(void * ctx);
