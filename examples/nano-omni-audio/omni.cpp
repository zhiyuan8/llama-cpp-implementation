...
static bool omni_eval_audio_embed(llama_context *ctx_llama, ggml_tensor *audio_embed, int n_batch, int *n_past)
{
    ...
    printf("omni_eval_audio_embed details:\n");
    printf("- n_embd: %d\n", n_embd);
    printf("- n_audio_embed: %d\n", n_audio_embed);
    printf("- tensor dim0 (should match n_embd): %d\n", audio_embed->ne[0]);

    GGML_ASSERT(audio_embed->ne[0] == n_embd);

    size_t audio_embed_size = ggml_nbytes(audio_embed);
    printf("- audio_embed_size: %zu bytes\n", audio_embed_size);
    ...
    printf("First 5 values of audio embedding:\n");
    for (int i = 0; i < 5 && i < n_embd; i++) {
        printf("%f ", audio_embed_data[i]);
    }
    
    printf("\nContext validation:\n");
    printf("- ctx_llama pointer: %p\n", (void*)ctx_llama);
    if (ctx_llama) {
        printf("- model pointer: %p\n", (void*)llama_get_model(ctx_llama));
    } else {
        printf("WARNING: ctx_llama is NULL!\n");
        return false;
    }
    ...
    for (int i = 0; i < n_audio_embed; i += n_batch)
    {
        ...
        printf("\nBatch details (iteration %d):\n", i/n_batch);
        printf("- n_eval: %d\n", n_eval);
        printf("- current n_past: %d\n", *n_past);
        ...
        printf("Batch struct validation:\n");
        printf("- n_tokens: %d\n", batch.n_tokens);
        printf("- embd pointer: %p\n", (void*)batch.embd);
        printf("- all_pos_0: %d\n", batch.all_pos_0);
        printf("- all_pos_1: %d\n", batch.all_pos_1);
        printf("- all_seq_id: %d\n", batch.all_seq_id);
...
ggml_tensor *omni_process_audio(struct omni_context *ctx_omni, omni_params &params)
{
    ...
    printf("Encoding audio with Whisper...\n");
...
void omni_process_prompt(struct omni_context *ctx_omni, ggml_tensor *audio_embed, omni_params &params, const std::string &prompt)
{
    ...
    {
        printf("AUDIO_TOKEN found in prompt. Using custom system and user prompts.\n");
    ...
    printf("finished eval_string\n");

    // Add debug prints before omni_eval_audio_embed
    printf("=== Debug omni_eval_audio_embed params ===\n");
    printf("- audio_embed dimensions: [%d, %d]\n", audio_embed->ne[0], audio_embed->ne[1]);
    printf("- n_batch: %d\n", params.gpt.n_batch);
    printf("- n_past: %d\n", n_past);

    omni_eval_audio_embed(ctx_omni->ctx_llama, audio_embed, params.gpt.n_batch, &n_past);
    printf("finished omni_eval_audio_embed\n");
